...
def money_column(db_name = "island_game.db"):
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    cursor.execute("PRAGMA table_info(island)")
    columns = [row[1] for row in cursor.fetchall()]
    if "money" not in columns:
        cursor.execute("ALTER TABLE island ADD COLUMN money INTEGER DEFAULT 0")
    conn.commit()
    conn.close()
...
def save_game_to_db(island, islanders, dailies_food, money, db_name="island_game.db"):
    ...
    # Save island details
    cursor.execute('INSERT OR REPLACE INTO island (name, last_login, dailies_food, money) VALUES (?, ?, ?, ?)', 
